...
init_obs() {
  ...
  if [ ! -f "$STATE_LOG" ]; then
    printf '{"current_task":null,"stuck_count":0,"last_successful":null,"total_failed":0,"created_at":"%s"}\n' "$(date -Iseconds)" > "$STATE_LOG"
  fi
}
...
update_state() {
  ...
  printf '{"current_task":"%s","stuck_count":%d,"last_successful":"%s","total_failed":%d,"updated_at":"%s"}\n' \
    "$current_task" "$stuck_count" "$last_successful" "$total_failed" "$(date -Iseconds)" \
    > "$STATE_LOG"
}
